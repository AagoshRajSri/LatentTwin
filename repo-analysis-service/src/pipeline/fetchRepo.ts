...
export async function fetchRepo(
  ...
  if (meta.sizeKb > MAX_REPO_SIZE_KB) {
    throw new Error(
      `Repo is ${Math.round(meta.sizeKb / 1024)} MB — exceeds the configured limit of ${MAX_REPO_SIZE_MB} MB.`,
    );
  }
  ...
  if (!tree) {
    throw new Error(
      `Failed to retrieve repository tree. Please verify the repository URL and your GITHUB_TOKEN.`,
    );
  }
  ...
  if (sourceFiles.length > MAX_FILES) {
    throw new Error(
      `Repo has ${sourceFiles.length.toLocaleString()} source files — exceeds the ${MAX_FILES.toLocaleString()} file limit.`,
    );
  }

  // Fetch all source file contents via REST API
  try {
    const files = await fetchContentsViaApi(
      meta.owner,
      meta.name,
      meta.commitSha,
      sourceFiles,
    ...
    throw new Error(
      `Failed to fetch file contents: ${err instanceof Error ? err.message : "Unknown API error"}. Installing Git on the host system or configuring a valid GITHUB_TOKEN may resolve this.`,
    );
  }
}
  ...
  for (let i = 0; i < treeFiles.length; i += BATCH) {
    ...
    for (const r of settled) {
      if (r.status === "fulfilled" && r.value) results.push(r.value);
        ...
        failures.push(
          r.status === "rejected" ? String(r.reason) : "empty response",
        );
    }
  }
  if (failures.length > 0) {
    throw new Error(
      `Failed to fetch ${failures.length} repository file(s); refusing to analyze an incomplete checkout.`,
    );
  }
  return results;
}
...
async function cloneAndRead(
  ...
  try {
    ...
    if (files.length > MAX_FILES) {
      ...
      throw new Error(
        `Repo has ${files.length.toLocaleString()} files — exceeds the ${MAX_FILES.toLocaleString()} limit.`,
      );
    }
