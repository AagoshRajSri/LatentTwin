...
function FlowContent() {
    ...
    const fetchGraph = async () => {
      ...
      try {
        ...
        const rfEdges = data.edges.map((edge) => {
          const isImplicit =
            edge.relationshipType === "implicit_queue" ||
            edge.relationshipType === "implicit_db" ||
            edge.type === "implicit" ||
            edge.type === "subscribes";
          return {
    ...
    if (isDemo) {
        ...
        relevantInvariants: [
          {
            pr: "PR#42",
            description:
              "Field renames must propagate to all consumers before merge.",
          },
          {
            pr: "PR#61",
            description:
              "Event schema types must match producer payload shape.",
          },
        ],
      };
      ...
      setTimeout(() => {
        ...
        setApplyResult({
          ...
          output: "All field references updated. Tests passing.",
        });
    ...
    try {
      ...
      setNodes((prevNodes) =>
        prevNodes.map((node) => ({
          ...
          data: {
            ...node.data,
            isImpacted: affectedSet.has(node.id),
            isTarget: node.id === data.target,
          },
        })),
      );
      setEdges((prevEdges) =>
    ...
    try {
      ...
      if (data.status === "SYSTEM HEALED") {
        ...
        setEdges((prevEdges) =>
          prevEdges.map((edge) => {
            const isImplicit =
              edge.data?.relationshipType === "implicit_queue" ||
              edge.data?.relationshipType === "implicit_db" ||
              edge.data?.type === "implicit" ||
              edge.data?.type === "subscribes";
            return {
              ...edge,
              animated: isImplicit,
              style: {
  ...
  const displayNodes = useMemo(() => {
    ...
    return nodes.map((n) => {
      let codeHaystack = [];
      if (n.data?.lines) {
        codeHaystack = n.data.lines.flatMap((l) => [
          l.code,
          l.before,
          l.after,
          l.hint,
  ...
  const handleSearchEnter = useCallback(
    (e) => {
      ...
      if (hit)
        fitView({ duration: 600, padding: 0.3, nodes: [{ id: hit.id }] });
    },
        ...
        >
          {showMascot ? (
            ...
            <img
              src="/logo.png"
              alt="LatentTwin Logo"
              className="w-11 h-11 object-contain rounded-xl shadow-lg shadow-blue-500/20 group-hover:scale-105 transition-transform"
            />
          )}
        ...
        <div className="flex items-center justify-end gap-3 shrink-0">
          ...
          {analysisSnapshot && !analyzing && (
            <div className="flex items-center gap-1.5 bg-gray-900/60 p-1 rounded-lg border border-gray-800/60 shadow-inner mr-2">
              ...
              <span className="text-[9px] font-bold uppercase tracking-widest text-gray-500 mr-1 hidden lg:inline">
                Export
              </span>
              <button
                id="export-json-btn"
        ...
        {viewMode === "3d" ? (
          ...
          <>
            ...
            <div className="flex-1 h-full bg-grid-pattern relative">
              ...
              {!loading && (
                <div
                  className="absolute bottom-6 left-1/2 z-30 flex items-center gap-1.5 bg-gray-900/95 border border-gray-700/80 rounded-xl px-2 py-1.5 shadow-2xl backdrop-blur-md"
                  style={{ transform: "translateX(-50%)" }}
                >
                  {/* Zoom out */}
                  <button
                  ...
                  >
                    <svg
                      ...
                      strokeWidth="2.5"
                    >
                      <path d="M3 9V5a2 2 0 0 1 2-2h4M3 15v4a2 2 0 0 0 2 2h4m10-16h4a2 2 0 0 1 2 2v4m0 10v4a2 2 0 0 1-2 2h-4" />
                    </svg>
                    Fit All
                  </button>
                  <div className="w-px h-4 bg-gray-700" />
              ...
              {loading ? (
                ...
                <div className="absolute inset-0 z-10 flex items-center justify-center px-6">
                  <div className="max-w-md rounded-xl border border-gray-800 bg-gray-950/90 px-6 py-5 text-center shadow-2xl backdrop-blur-md">
                    ...
                    <button
                      onClick={loadDemo}
                      className="mt-4 rounded-md bg-emerald-700 px-4 py-2 text-xs font-semibold text-white transition hover:bg-emerald-600"
                    >
                      Load Demo Graph
                    </button>
                  </div>
            ...
            <div className="absolute top-6 bottom-20 left-6 z-20 flex flex-col gap-3 w-72 max-h-full pointer-events-none pb-2">
              ...
              <div
                className="flex flex-col gap-3 overflow-y-auto hide-scrollbar"
                style={{ maxHeight: "100%" }}
              >
                    ...
                    >
                      {simulating ? (
                        <>
                          ...
                          />
                          <span>Simulating Break...</span>
                        </>
                      ) : (
                        <>
                          <AlertTriangle size={13} />
                          <span>Inject Breaking Change</span>
                        </>
                      )}
                    </button>
                  </div>
                ...
                {simulationResult &&
                  applyResult?.status !== "SYSTEM HEALED" && (
                    <div className="bg-red-950/90 border border-red-900/50 p-3.5 rounded-xl shadow-xl animate-fade-in flex flex-col gap-3 pointer-events-auto shrink-0 backdrop-blur-sm">
                      ...
                      <div>
                        ...
                        <div className="flex flex-col gap-1">
                          {simulationResult.affectedNodes
                            ?.filter(
                              (nodeId) => nodeId !== simulationResult.target,
                            )
                            .map((nodeId) => {
                              ...
                              return (
                                <div
                                  key={nodeId}
                                  className="flex items-center gap-2 text-[11px] text-red-300 bg-red-900/20 border border-red-900/30 px-2 py-1 rounded font-medium"
                                >
                      ...
                      {simulationResult.relevantInvariants &&
                        simulationResult.relevantInvariants.length > 0 && (
                          <div className="border-t border-red-900/50 pt-2.5">
                            <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                              <GitMerge size={11} className="text-gray-400" />{" "}
                              Invariants
                            </h3>
                            <div className="flex flex-col gap-1.5">
                              {simulationResult.relevantInvariants.map(
                                (inv, idx) => (
                                  <div
                                    key={idx}
                                    className="bg-gray-900/60 border border-gray-800 px-2 py-1.5 rounded text-[11px] text-gray-400 leading-relaxed"
                                  >
            ...
            {(isDemo || analysisSnapshot) && (
              <div className="absolute bottom-6 left-6 z-30 pointer-events-auto group">
                ...
                <div
                  className="absolute bottom-full left-0 mb-2 w-48 bg-gray-900/95 border border-gray-800 p-3.5 rounded-xl shadow-2xl backdrop-blur-sm
                            opacity-0 translate-y-2 pointer-events-none
                            group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto
                            transition-all duration-200 ease-out"
                >
                  <h3 className="text-[10px] font-bold text-gray-500 mb-2.5 uppercase tracking-widest">
                    Dependency Types
                  </h3>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto p-6">
                  {loadingRepair ? (
                    ...
                    <div className="flex flex-col gap-6">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-gray-900 border border-gray-800 p-3 rounded">
                          ...
                          <span className="text-gray-200 text-xs font-semibold">
                            {repairData.affectedService}
                          </span>
                        </div>
                      </div>

                      <div>
                        <h3 className="text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest">
                          Dependency Trace
                        </h3>
                        <div className="flex items-center gap-2 text-xs font-mono bg-gray-900 p-3 rounded border border-gray-800 overflow-x-auto text-gray-400 whitespace-nowrap">
                          {repairData.dependencyPath.map((node, i) => (
                            <React.Fragment key={node}>
                              <span>{node}</span>
                              {i < repairData.dependencyPath.length - 1 && (
                      ...
                      <div>
                        ...
                        <div className="flex flex-col gap-2">
                          {repairData.historicalInvariants.map((inv, i) => (
                            <div
                              key={i}
                              className="text-xs text-gray-300 bg-gray-900 border border-gray-800 p-3 rounded flex flex-col gap-1.5 leading-relaxed"
                            >
                      ...
                      {applyResult && (
                        <div
                          className={`mt-2 p-4 rounded border ${applyResult.status === "SYSTEM HEALED" ? "bg-green-950/50 border-green-900/50" : "bg-red-950/50 border-red-900/50"}`}
                        >
                          <h3
                            className={`text-xs uppercase tracking-widest font-bold mb-2 flex items-center gap-2 ${applyResult.status === "SYSTEM HEALED" ? "text-green-500" : "text-red-500"}`}
                          >
                            {applyResult.status === "SYSTEM HEALED" && (
                          ...
                          {applyResult.validationResult && (
                            <div className="mt-4 pt-4 border-t border-gray-800/50">
                              ...
                              <pre className="text-[10px] font-mono bg-gray-950 p-3 rounded border border-gray-800 overflow-x-auto max-h-48 text-gray-400">
                                {applyResult.validationResult.stdout ||
                                  applyResult.validationResult.stderr ||
                                  "No output"}
                              </pre>
                            </div>
                          )}
                  ...
                  <div className="flex-shrink-0 p-4 border-t border-gray-800 bg-gray-900 flex justify-end gap-3 sticky bottom-0">
                    ...
                    >
                      {applyingPatch ? (
                        ...
                        applyResult.status === "REPAIR FAILED" ? (
                        "Retry Patch"
                      ) : applyResult &&
                        applyResult.status === "SYSTEM HEALED" ? (
                  ...
                  <div className="w-[420px] border-l border-gray-800 bg-gray-900 overflow-y-auto flex flex-col shadow-2xl z-20">
                    ...
                    >
                      ...
                      <button
                        ...
                        className="text-gray-600 hover:text-white transition shrink-0 ml-2"
                      >
                        <X size={16} />
                      </button>
                    </div>
                      ...
                      {bugLines.length > 0 && (
                        <div className="px-4 py-3 border-b border-gray-800/60">
                          <h3 className="text-[10px] font-bold text-red-400 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                            <AlertCircle size={11} /> {bugLines.length} Bug
                            {bugLines.length > 1 ? "s" : ""} Detected
                          </h3>
                          <div className="flex flex-col gap-2">
                            {bugLines.map((line, i) => (
                              <div
                              ...
                              >
                                {line.lineNumber && (
                                  <p className="text-[10px] text-red-500 font-mono mb-1">
                                    Line {line.lineNumber}
                                  </p>
                                )}
                                {line.before && (
                                  <pre className="text-[11px] font-mono text-red-300 bg-red-950/40 px-2 py-1 rounded mb-1 whitespace-pre-wrap break-all">
                                    {line.before}
                                  </pre>
                                )}
                                {line.hint && (
                                ...
                                {line.after && (
                                  <div className="mt-2">
                                    <p className="text-[10px] text-emerald-500 font-bold mb-0.5">
                                      Suggested Fix:
                                    </p>
                                    <pre className="text-[11px] font-mono text-emerald-300 bg-emerald-950/30 px-2 py-1 rounded whitespace-pre-wrap break-all">
                                      {line.after}
                                    </pre>
                                  </div>
                                )}
                          ...
                          <div className="mb-2">
                            ...
                            {outEdges.map((e) => {
                              ...
                              return (
                                <div
                                  key={e.id}
                                  className={`flex items-center justify-between text-xs px-2 py-1 rounded mb-1 ${
                                    tgtStatus === "impacted"
                                      ? "bg-red-950/40 border border-red-900/40 text-red-300"
                                      : tgtStatus === "affected-downstream"
                                        ? "bg-orange-950/30 border border-orange-900/30 text-orange-300"
                                ...
                                >
                                  ...
                                  {tgtStatus === "impacted" && (
                                    <AlertCircle
                                      size={10}
                                      className="text-red-500 shrink-0 ml-1"
                                    />
                                  )}
                                </div>
                              );
