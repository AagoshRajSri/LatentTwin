...
export default function CrossSectionNode({ data, id }) {
  ...
  const hasImpacted = layers.some((l) =>
    (l.lines ?? []).some((ln) => ln.status === "error"),
  );
  const allResolved = layers.every((l) =>
    (l.lines ?? []).every((ln) => ln.status !== "error"),
  );
  const nodeStatus =
    data?.status ??
    (!hasImpacted ? "healthy" : allResolved ? "resolved" : "impacted");

  const layerHasError = useCallback(
    (li) => {
    ...
    <div className="csn-scene">
      ...
      <div className="csn-shell">
        <div
          ...
          onClick={() =>
            setMode((m) => (m === "collapsed" ? "z" : "collapsed"))
          }
                  ...
                  {Array.from({ length: layers.length - 1 }, (_, i) => (
                    <div
                      ...
                      className={`csn-connector conn-${mode} conn-${i} ${
                        layerHasError(i) || layerHasError(i + 1)
                          ? "conn-error"
                          : "conn-ok"
                      }`}
                    >
...
function LayerCard({
  ...
  const resolvedRecently =
    safeLines.some((l) => l.status === "resolved") && !broken;
  ...
  return (
    ...
    >
      <div className="csn-layer-header">
        ...
        <span
          className={`csn-layer-status-tag ${broken ? "error" : resolvedRecently ? "resolved" : "ok"}`}
        >
      ...
      <div className="csn-line-list">
        {safeLines.map((line, lni) => {
          ...
          return (
            <div
              key={line.id}
              className={`csn-line-item ${line.status === "resolved" ? "resolved-line" : ""}`}
            >
              <span className="csn-line-gutter">{lni + 1}</span>
