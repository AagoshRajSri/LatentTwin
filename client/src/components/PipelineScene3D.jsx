...
function makeLabelTexture(label, glyph, accentHex) {
  ...
  if (glyph === "server") {
    for (let i = 0; i < 3; i++) {
      ...
      ctx.beginPath();
      ctx.arc(-35 * s, (-55 + i * 46) * s, 4 * s, 0, Math.PI * 2);
      ctx.fill();
    }
  } else if (glyph === "database") {
    for (let i = 0; i < 3; i++) {
      ctx.beginPath();
      ctx.ellipse(0, (-70 + i * 45) * s, 60 * s, 20 * s, 0, 0, Math.PI * 2);
      ctx.stroke();
    }
    ctx.beginPath();
...
export default function PipelineScene3D({ analysisData, demoMode = false }) {
  ...
  useEffect(() => {
    ...
    scene.add(rim);
    const under = new THREE.DirectionalLight(0x334066, 0.25);
        ...
        tier.files.forEach((f, fi) => {
          ...
          const chipEdge = new THREE.LineSegments(
            ...
            new THREE.LineBasicMaterial({
              color: isErr ? 0xffb3b4 : isDownstream ? 0xfed7aa : 0x8fa0c8,
            }),
          );
          mesh.add(chipEdge);
          mesh.userData = {
    ...
    for (const [linkIndex, link] of links.entries()) {
      ...
      const to = toInfo?.node || link.to;
      if (!from || !to) continue;
      const p0 = fromInfo
        ? fromInfo.mesh.getWorldPosition(new THREE.Vector3())
        : new THREE.Vector3(from.x + 1.35, -0.35, 0);
      const p3 = toInfo
